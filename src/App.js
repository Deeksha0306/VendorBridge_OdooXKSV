...
function Login({ navigate, setUser }) {
  ...
  const handleLogin = () => {
    if (email === 'admin@vendor.com' && password === 'admin123') {
      setUser({ name: 'Admin User', role });
      navigate('dashboard');
    } else {
      setError('Use admin@vendor.com / admin123');
    }
  };
...
function Vendors({ navigate }) {
  ...
  const vendors = [
    { id: 'V001', name: 'TechCorp Solutions', category: 'IT Equipment', gst: '27AABCT1332L1ZG', contact: 'tech@techcorp.com', status: 'Active', rating: '4.5⭐' },
    { id: 'V002', name: 'Office Essentials', category: 'Office Supplies', gst: '27AABCO1234L1ZG', contact: 'info@officeessentials.com', status: 'Active', rating: '4.2⭐' },
    { id: 'V003', name: 'BuildRight Infra', category: 'Construction', gst: '27AABCB5678L1ZG', contact: 'build@buildright.com', status: 'Inactive', rating: '3.8⭐' },
    { id: 'V004', name: 'SwiftLogistics', category: 'Logistics', gst: '27AABCS9012L1ZG', contact: 'swift@logistics.com', status: 'Active', rating: '4.7⭐' },
  ];
    ...
    <div style={s.page}>
      ...
      <div style={s.content}>
        <div style={s.topBar}>
          ...
          <button style={s.btnPrimary}>+ Add Vendor</button>
        </div>
        <input style={{ ...s.searchInput }} placeholder="🔍  Search vendors by name or category..." value={search} onChange={e => setSearch(e.target.value)} />
        <div style={s.tableWrap}>
          <table style={s.table}>
            <thead>
              <tr>{['Vendor ID', 'Name', 'Category', 'GST Number', 'Contact', 'Rating', 'Status'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.map(v => (
                <tr key={v.id} style={s.tr}>
                  <td style={s.td}><span style={{ color: '#818cf8', fontWeight: 600 }}>{v.id}</span></td>
                  ...
                  <td style={s.td}><span style={{ ...s.badge, background: v.status === 'Active' ? '#065f46' : '#7f1d1d', color: v.status === 'Active' ? '#6ee7b7' : '#fca5a5' }}>{v.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function RFQ({ navigate }) {
  const rfqs = [
    { id: 'RFQ001', title: 'Office Laptops Q1', product: 'Laptops', qty: 20, deadline: '2024-02-15', vendors: 'TechCorp, DigiWorld', status: 'Open' },
    { id: 'RFQ002', title: 'Office Furniture', product: 'Chairs & Desks', qty: 50, deadline: '2024-02-20', vendors: 'Office Essentials', status: 'Open' },
    { id: 'RFQ003', title: 'Server Infrastructure', product: 'Servers', qty: 5, deadline: '2024-02-10', vendors: 'TechCorp', status: 'Closed' },
  ];
  return (
    <div style={s.page}>
      ...
      <div style={s.content}>
        <div style={s.topBar}>
          ...
          <button style={s.btnPrimary}>+ Create RFQ</button>
        </div>
        <div style={s.tableWrap}>
          <table style={s.table}>
            <thead>
              <tr>{['RFQ ID', 'Title', 'Product', 'Quantity', 'Deadline', 'Assigned Vendors', 'Status', 'Action'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {rfqs.map(r => (
                <tr key={r.id} style={s.tr}>
                  <td style={s.td}><span style={{ color: '#818cf8', fontWeight: 600 }}>{r.id}</span></td>
    ...
    <div style={s.page}>
      ...
      <div style={s.content}>
        ...
        <div style={{ background: '#1e1b4b', border: '1px solid #3730a3', borderRadius: 10, padding: '12px 16px', marginBottom: 20 }}>
          <p style={{ color: '#a5b4fc', margin: 0 }}>💡 Tip: Green highlighted row has the lowest total price. Click Select to proceed to approval.</p>
        </div>
        <div style={s.tableWrap}>
          <table style={s.table}>
            <thead>
              <tr>{['Vendor', 'Base Price', 'Tax (18%)', 'Total Amount', 'Delivery', 'Rating', 'Action'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {quotes.map(q => (
                <tr key={q.vendor} style={{ background: q.total === lowest ? '#052e16' : '' }}>
                  ...
                  <td style={{ ...s.td, fontWeight: 700, color: q.total === lowest ? '#6ee7b7' : '#f1f5f9', fontSize: 15 }}>₹{q.total.toLocaleString()}</td>
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.content}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ color: '#94a3b8', margin: 0, fontSize: 14 }}>
                  🏭 Vendor: <strong style={{ color: '#e2e8f0' }}>{r.vendor}</strong> &nbsp;|&nbsp;
                  💰 Amount: <strong style={{ color: '#34d399' }}>{r.amount}</strong> &nbsp;|&nbsp;
                  👤 By: {r.requestedBy} &nbsp;|&nbsp;
                  📅 {r.date}
                </p>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                {status[r.id] ? (
                  ...
                  <>
                    <button style={{ ...s.btnPrimary, background: 'linear-gradient(135deg, #065f46, #047857)' }} onClick={() => setStatus({ ...status, [r.id]: 'Approved' })}>✅ Approve</button>
                    <button style={{ ...s.btnPrimary, background: 'linear-gradient(135deg, #7f1d1d, #991b1b)' }} onClick={() => setStatus({ ...status, [r.id]: 'Rejected' })}>❌ Reject</button>
                  </>
                )}
              </div>
            </div>
        ...
        <button style={{ ...s.btnPrimary, marginTop: 8 }} onClick={() => navigate('po')}>Generate Purchase Order →</button>
      </div>
    </div>
  );
}
        ...
        <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 12, padding: 32, maxWidth: 750 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 28, paddingBottom: 20, borderBottom: '1px solid #334155' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <h3 style={{ color: '#f1f5f9', margin: 0, fontSize: 20 }}>PURCHASE ORDER</h3>
              <p style={{ color: '#818cf8', margin: '4px 0', fontWeight: 700 }}>{po.poNumber}</p>
          ...
          <div style={{ textAlign: 'right', marginTop: 20, paddingTop: 16, borderTop: '1px solid #334155' }}>
            <p style={{ color: '#94a3b8' }}>Subtotal: ₹{po.items[0].total.toLocaleString()}</p>
          ...
          <button style={{ ...s.btnPrimary, marginTop: 16 }} onClick={() => navigate('invoice')}>Generate Invoice →</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={s.content}>
        <div style={s.topBar}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button style={s.btnPrimary} onClick={() => window.print()}>🖨️ Print Invoice</button>
            <button style={{ ...s.btnPrimary, background: 'linear-gradient(135deg, #065f46, #047857)' }}>📧 Send via Email</button>
          </div>
        </div>
        <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 12, padding: 32, maxWidth: 750 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 28, paddingBottom: 20, borderBottom: '1px solid #334155' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <h3 style={{ color: '#f1f5f9', margin: 0, fontSize: 20 }}>INVOICE</h3>
              <p style={{ color: '#818cf8', margin: '4px 0', fontWeight: 700 }}>{inv.invoiceNumber}</p>
          ...
          <div style={{ textAlign: 'right', marginTop: 20, paddingTop: 16, borderTop: '1px solid #334155' }}>
            <p style={{ color: '#94a3b8' }}>Subtotal: ₹{inv.items[0].total.toLocaleString()}</p>
...
const s = {
  ...
  navBtn: { display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', color: '#94a3b8', padding: '11px 14px', textAlign: 'left', cursor: 'pointer', fontSize: 14, marginBottom: 4, borderRadius: 8, width: '100%', transition: 'all 0.2s' },
